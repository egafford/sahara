...
class TestImages(b.SaharaTestCase):
    ...
    def test_script_spec(self):
        ...
        resource_roots = ['etc/edp-examples/edp-shell']

        validator = cls.from_spec('shell-example.sh', {}, resource_roots)
        self.assertIsInstance(validator, cls)
        self.assertIn("EXTRA_FILE", validator.script_contents)
        self.assertEqual(validator.env_vars, [])

        validator = cls.from_spec(
            {'shell-example.sh': {'env_vars': ['EXTRA_FILE', 'USER']}},
            {}, resource_roots)
        self.assertIsInstance(validator, cls)
        self.assertIn("EXTRA_FILE", validator.script_contents)
        self.assertEqual(validator.env_vars, ['EXTRA_FILE', 'USER'])
    ...
    def test_package_validator_debian(self):
        ...
        try:
            ...
            self.assertEqual(e.message, "So bad!")
        remote.execute_command.assert_called_with(
            "dpkg -s java-8\ndpkg -s hadoop", run_as_root=True)
        self.assertEqual(remote.execute_command.call_count, 1)
    ...
    def test_any_validator(self):
        ...
        always_lies = FakeValidator(mock.Mock(side_effect=Exception()))
    ...
    def test_all_validator(self):
        ...
        always_lies = mock.Mock(validate=mock.Mock(
            side_effect=exceptions.ImageValidationError("Boom!")))
